...
const ForgotPassword: React.FC = () => {
  ...
  const requestCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMsg("A reset code has been sent to your email. Please enter it below along with a new password.");
        setStep("reset");
      } else {
        setErr("Could not send reset code");
      }
    } catch {
  ...
  const resetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErr(j?.error || "Reset failed. Code may be invalid or expired.");
      }
    } catch {
      setErr("Network error");
    } finally {
      ...
      <div className="w-full max-w-md bg-card p-8 rounded-2xl shadow-card">
        ...
        {step === "reset" && (
          <form onSubmit={resetPassword} className="space-y-3">
            <p className="text-sm text-muted-foreground mb-4">
              Enter the reset code you received and set your new password.
            </p>
            {/* Email field (locked) */}
            <input
              disabled
