...
const ShoppingList: React.FC<ShoppingListProps> = ({ onOptimize }) => {
  ...
  const categories = ['Groceries', 'Dairy', 'Meat', 'Vegetables', 'Fruits', 'Beverages', 'Household'];
      ...
      <CardContent className="p-6">
        <div className="flex gap-3 mb-6">
          ...
          <Input
            ...
            className="w-20"
          />
          <Button onClick={addItem} variant="default" className="bg-fresh hover:bg-fresh/90 shadow-button">
        ...
        {items.length > 0 && (
          <div className="space-y-3 mb-6">
            ...
            {items.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-3 bg-gradient-card rounded-lg border">
        ...
        {items.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">We'll help you find the best deals across multiple stores!</p>
          </div>
        )}
      </CardContent>
    </Card>
